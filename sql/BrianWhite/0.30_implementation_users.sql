...
BEGIN
	...
	INSERT INTO implementation_users
		(
		SAContactID, SAUserID, StaffCode, full_name, SALoginID, Prefix, SAFirst, SAMiddle, SALast, Suffix
		)
		SELECT
			NULL						   AS SAContactID
		   ,NULL						   AS SAUserID
		   ...
		   ,suf.name					   AS Suffix
		FROM BrianWhiteNeos.[dbo].[staff] s
		JOIN brianwhiteneos.dbo.prefix p
			ON p.id = s.prefixid
		JOIN BrianWhiteNeos.dbo.suffix suf
			ON s.suffixid = suf.id
END
ELSE
IF @Phase = 2
BEGIN
	-- Phase 2: Use implementation database as starting point and add staff_code from [staff]
	-- at this point, the user table contains legit users entered by the client
	-- if staff code does not exist, that user was entered manually by the client
	INSERT INTO implementation_users
		(
		SAContactID, SAUserID, StaffCode, full_name, SALoginID, Prefix, SAFirst, SAMiddle, SALast, Suffix, Active, Visible
		)
		SELECT
			smic.cinnContactID
		   ,u.usrnUserID
		...
		FROM BrianWhiteSA..sma_mst_users u
		JOIN BrianWhiteSA..sma_MST_IndvContacts smic
			ON smic.cinnContactID = u.usrnContactID
		LEFT JOIN BrianWhiteNeos..staff s
